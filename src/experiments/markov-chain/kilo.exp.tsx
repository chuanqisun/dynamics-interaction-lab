...
export class BusinessProcessExperiment extends React.Component<any, any> {
  ...
  constructor(props: any) {
    ...
    this.state = {
      ...
      stages: [{ name: "Qualify" }, { name: "Develop" }, { name: "Propose" }, { name: "Wrap up" }],
      userSelectedIndex: 0,
      recordAtIndex: 0,
      ...
      /*dialog*/
      dialogElement: null,
      switchProcessOptions: ["Printer sales process", "Toner subscription sales process"],
  ...
  render() {
    const { selectedTabId, ...rest } = this.state;

    const businessProcessFlowProps = { ...rest, leadingProgressBarWidthInPixel: 20 };

    const formHeaderProps = {
      showFormSwitcher: false,
      fields: [
        { key: "Process started", value: "4 days ago", type: "text" },
        { key: "State started", value: "30 minutues ago", type: "text" },
        { key: "Opportunity", value: "Cooking Appliances IOT Capable", type: "link" },
      ],
      formSwitcherOptions: [],
      formSwitcherSelectedIndex: this.state.formSwitcherSelectedIndex,
      onFormSwitch: this.state.onFormSwitch,
      entityName: "Printer sales process",
      recordName: this.state.userSelectedIndex !== null ? this.state.stages[this.state.userSelectedIndex].name : "Finished",
    };
    const sideNavProps = {
      ...
      <StyledApp>
        ...
        <div className="bottom">
          ...
          <Scrollbars className="page-center" autoHide autoHideTimeout={1000} autoHideDuration={200}>
            <div className="page-center-top">
              <div className="command-bar-container">{commands}</div>
              <ProcessHeader className="form-header" inlineModeBreakpoint="600px" {...formHeaderProps} />
            ...
            <div className="page-center-bottom">
              <div className={`form-tab${this.state.selectedTabId === "-1" ? " form-tab--process" : ""}`}>
                ...
                {this.state.selectedTabId === "-1" && (
                  <div className="form-section form-section--process">
                    <div className="ft">
                      ...
                      <span className="switch-process-inline">
                        {" "}
                        (
                        <button className="switch-process-btn" onClick={this.onOpenSwitchProcessDialog}>
                          switch
                        </button>
                        )
                      </span>
                    ...
                    {this.state.userSelectedIndex < this.state.recordAtIndex && (
                      <span className="process-attribute">Stage completed {14 - 2 * this.state.userSelectedIndex} minutes ago</span>
                    )}
                    {this.state.userSelectedIndex === this.state.recordAtIndex && (
                      <span className="process-attribute">Stage active for {9 - 2 * this.state.userSelectedIndex} minutes</span>
                    )}

                    <div className="process-actions">
                      {this.state.userSelectedIndex < this.state.recordAtIndex && (
                        <button className="process-action-btn" onClick={() => this.onMoveRecordToStage(this.state.userSelectedIndex)}>
                          Rollback to “{this.state.stages[this.state.userSelectedIndex].name}”
                        </button>
                      )}
                      {this.state.userSelectedIndex === this.state.recordAtIndex && this.state.recordAtIndex < this.state.stages.length - 1 && (
                        <button className="process-action-btn process-action-btn--primary" onClick={this.onCompleteStage}>
                          Complete stage
                        </button>
                      )}
                      {this.state.userSelectedIndex === this.state.recordAtIndex && this.state.recordAtIndex === this.state.stages.length - 1 && (
                ...
                {/*BPF: Qualify stage*/}
                {this.state.selectedTabId === "-1" && this.state.userSelectedIndex === 0 && (
                  <form ref={this.qualifyStageFormRef} className="form-section">
                    <div className="ff">
                      <label htmlFor="contact" className="ff__key">
                        Existing contact
                      </label>
                      <a id="contact" className="ff__value">
                        {this.state.contact}
                      </a>
                    </div>
                    <div className="ff">
                      <label htmlFor="account" className="ff__key">
                        Existing account
                      </label>
                      <a id="account" className="ff__value">
                        {this.state.account}
                      </a>
                    </div>
                    <div className="ff">
                      <label htmlFor="purchaseTimeframe" className="ff__key">
                        Purchase timeframe
                      </label>
                      <select
                        id="purchaseTimeframe"
                        className="ff__value"
                        value={this.state.purchaseTimeframe}
                        onChange={(e) => this.setState({ purchaseTimeframe: e.target.value })}
                      >
                    ...
                    <div className="ff">
                      <label htmlFor="estimatedBudgetRef" className="ff__key">
                        Estimated budget<span className="ff__asterisk">*</span>
                      </label>
                      <input
                        id="estimatedBudgetRef"
                        className="ff__value"
                        ref={this.estimatedBudgetRef}
                        required
                        type="text"
                        placeholder="---"
                        value={this.state.estimatedBudget}
                        onChange={(e) => this.setState({ estimatedBudget: e.target.value })}
                      />
                    </div>
                    <div className="ff">
                      <label htmlFor="purchaseProcess" className="ff__key">
                        Purchase process
                      </label>
                      <select
                        id="purchaseProcess"
                        className="ff__value"
                        value={this.state.purchaseProcess}
                        onChange={(e) => this.setState({ purchaseProcess: e.target.value })}
                      >
                        <option value="" disabled>
                          --select--
                        </option>
                    ...
                    <div className="ff">
                      <label htmlFor="identifyDecisionMaker" className="ff__key">
                        Identify decision maker
                      </label>
                      <input
                        id="identifyDecisionMaker"
                        className="ff__value"
                        type="checkbox"
                        checked={this.state.identifyDecisionMaker}
                        onChange={(e) => this.setState({ identifyDecisionMaker: e.target.checked })}
                      />
                    </div>
                    <div className="ff">
                      <label htmlFor="captureSummary" className="ff__key">
                        Capture summary
                      </label>
                      <textarea
                        id="captureSummary"
                        className="ff__value"
                        placeholder="---"
                        value={this.state.captureSummary}
                        onChange={(e) => this.setState({ captureSummary: e.target.value })}
                      />
                    </div>
                  </form>
                )}
                {/*BPF: Develop stage */}
                {this.state.selectedTabId === "-1" && this.state.userSelectedIndex === 1 && (
                  <div className="form-section">
                    <div className="ff">
                      <label htmlFor="customerNeed" className="ff__key">
                        Customer need
                      </label>
                      <input
                        id="customerNeed"
                        ...
                        type="text"
                        placeholder="---"
                        value={this.state.customerNeed}
                        onChange={(e) => this.setState({ customerNeed: e.target.value })}
                      />
                    </div>
                    <div className="ff">
                      <label htmlFor="proposedSolution" className="ff__key">
                        Proposed solution
                      </label>
                      <input
                        id="proposedSolution"
                        ...
                        type="text"
                        placeholder="---"
                        value={this.state.proposedSolution}
                        onChange={(e) => this.setState({ proposedSolution: e.target.value })}
                      />
                    </div>
                    <div className="ff">
                      <label htmlFor="identifyStakeholders" className="ff__key">
                        Identify stakeholders
                      </label>
                      <input
                        id="identifyStakeholders"
                        className="ff__value"
                        type="checkbox"
                        checked={this.state.identifyStakeholders}
                        onChange={(e) => this.setState({ identifyStakeholders: e.target.checked })}
                      />
                    </div>
                    <div className="ff">
                      <label htmlFor="identifyCompetitors" className="ff__key">
                      ...
                      <input
                        ...
                        className="ff__value"
                        type="checkbox"
                        checked={this.state.identifyCompetitors}
                        onChange={(e) => this.setState({ identifyCompetitors: e.target.checked })}
                      />
                    </div>
                  </div>
                )}
                {/*BPF: Propose stage */}
                {this.state.selectedTabId === "-1" && this.state.userSelectedIndex === 2 && (
                  <div className="form-section">
                    <div className="ff">
                      <label htmlFor="identifySalesTeam" className="ff__key">
                        Identify sales team
                      </label>
                      <input
                        id="identifySalesTeam"
                        className="ff__value"
                        type="checkbox"
                        checked={this.state.identifySalesTeam}
                        onChange={(e) => this.setState({ identifySalesTeam: e.target.checked })}
                      />
                    </div>
                    <div className="ff">
                      <label htmlFor="developProposal" className="ff__key">
                        Develop proposal
                      </label>
                      <input
                        id="developProposal"
                        className="ff__value"
                        type="checkbox"
                        checked={this.state.developProposal}
                        onChange={(e) => this.setState({ developProposal: e.target.checked })}
                      />
                    </div>
                    <div className="ff">
                      <label htmlFor="completeInternalReview" className="ff__key">
                        Complete internal review
                      </label>
                      <input
                        id="completeInternalReview"
                        className="ff__value"
                        type="checkbox"
                        checked={this.state.completeInternalReview}
                        onChange={(e) => this.setState({ completeInternalReview: e.target.checked })}
                      />
                    </div>
                    <div className="ff">
                      <label htmlFor="presentPropsal" className="ff__key">
                        Present proposal
                      </label>
                      <input
                        id="presentPropsal"
                        className="ff__value"
                        type="checkbox"
                        checked={this.state.presentPropsal}
                        onChange={(e) => this.setState({ presentPropsal: e.target.checked })}
                      />
                    </div>
                  </div>
                )}
                {/*BPF: Close stage */}
                {this.state.selectedTabId === "-1" && this.state.userSelectedIndex === 3 && (
                  <div className="form-section">
                    <div className="ff">
                      <label htmlFor="completeFinalPropsal" className="ff__key">
                        Complete final proposal
                      </label>
                      <input
                        id="completeFinalPropsal"
                        className="ff__value"
                        type="checkbox"
                        checked={this.state.completeFinalPropsal}
                        onChange={(e) => this.setState({ completeFinalPropsal: e.target.checked })}
                      />
                    </div>
                    <div className="ff">
                      <label htmlFor="presentFinalProposal" className="ff__key">
                        Present final proposal
                      </label>
                      <input
                        id="presentFinalProposal"
                        className="ff__value"
                        type="checkbox"
                        checked={this.state.presentFinalProposal}
                        onChange={(e) => this.setState({ presentFinalProposal: e.target.checked })}
                      />
                    </div>
                    <div className="ff">
                      <label htmlFor="confirmDecisionDate" className="ff__key">
                        Confirm decision date
                      </label>
                      <input
                        id="confirmDecisionDate"
                        className="ff__value"
                        type="date"
                        value={this.state.confirmDecisionDate}
                        onChange={(e) => this.setState({ confirmDecisionDate: e.target.value })}
                      />
                    </div>
                    <div className="ff">
                      <label htmlFor="sendThankYou" className="ff__key">
                        Send thank you
                      </label>
                      <input
                        id="sendThankYou"
                        className="ff__value"
                        type="checkbox"
                        checked={this.state.sendThankYou}
                        onChange={(e) => this.setState({ sendThankYou: e.target.checked })}
                      />
                    </div>
                    <div className="ff">
                      <label htmlFor="fileDebrief" className="ff__key">
                        File de-brief
                      </label>
                      <input
                        id="fileDebrief"
                        className="ff__value"
                        type="checkbox"
                        checked={this.state.fileDebrief}
                        onChange={(e) => this.setState({ fileDebrief: e.target.checked })}
                      />
                    </div>
                  </div>
                )}
                {/* Lead form, Contact section */}
                {this.state.selectedTabId !== "-1" && this.state.recordAtIndex === 0 && (
                  <div className="form-section">
                    <div className="ft">Contact</div>
                    <div className="ff">
                      <label className="ff__key">
                        Topic<span className="ff__asterisk">*</span>
                      </label>
                ...
                {this.state.selectedTabId !== "-1" && this.state.recordAtIndex !== 0 && (
                  <div className="form-section">
                    ...
                    <div className="ff">
                      <label className="ff__key">
                        Currency<span className="ff__asterisk">*</span>
                      </label>
                ...
                {this.state.selectedTabId !== "-1" && (
                  <div className="form-section form-section--mock">
                    <Details />
                  </div>
                )}
                {this.state.selectedTabId !== "-1" && (
        ...
        {this.state.dialogElement && (
          <div className="dialog">
            <DialogElement state={this.state} />
          </div>
        )}
      </StyledApp>
    );
  ...
  onSwitchProcessSubmit = (e: any) => {
    ...
    this.setState({
      dialogElement: null,
      dialogSelectedProcessIndex: this.state.dialogSelectingProcessIndex,
      processName: this.state.switchProcessOptions[this.state.dialogSelectingProcessIndex],
    });
  };
  ...
  SwitchProcessDialog: React.FunctionComponent<any> = (state: any) => (
    <form className="dialog__box" onSubmit={(e) => this.onSwitchProcessSubmit(e)}>
      <h1 className="dialog__header">Choose a process</h1>
      <div className="dialog__main">
        {this.state.switchProcessOptions.map((option: string, index: number) => (
          <div className="switch-process-option" key={option}>
            <label>
              <input type="radio" checked={index === this.state.dialogSelectingProcessIndex} onChange={(e) => this.onSelectProcess(index)} />
              {option}
            </label>
          </div>
        ))}
      ...
      <div className="dialog__footer btn-group">
        ...
        <button className="btn btn--secondary" onClick={this.onSwitchProcessCancel}>
          Cancel
        </button>
      </div>
    </form>
