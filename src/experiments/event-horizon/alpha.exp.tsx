        ...
        <StyledNav>
          {this.state.stages.map((stage: any, index: number) => (
            <React.Fragment key={index}>
              <div className="node">
                <button
                  onClick={() => this.onSelectStage(index)}
                  className={
                    `btn${index < this.state.recordAtIndex ? " btn--shrinkable btn--filled" : ""}` +
              ...
              {index < this.state.stages.length - 1 ? (
                <div
                  className={`progress-bar${index < this.state.recordAtIndex ? " progress-bar--filled" : ""}${
                    index === this.state.recordAtIndex ? " progress-bar--fade" : ""
                  }`}
                />
              ) : null}
            </React.Fragment>
