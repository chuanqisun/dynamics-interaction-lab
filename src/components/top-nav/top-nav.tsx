...
export class TopNav extends React.Component<TopNavProps, TopNavState> {
  ...
  getRemainingSpace = (plan: ReflowPlan) => {
    ...
    const customAction = (plan.customOverflowStack.length === 1 && !plan.dynamicsOverflowStack.length && !plan.systemOverflowStack.length) ? constants.shellHeight : 0;
    ...
    return (
      <StyledHeader>
        ...
        <button className="shell-icon-button shell-icon-button--avatar">
          <img className="avatar" alt="frog" src={this.props.avatarImgUrl}></img>
        </button>
      </StyledHeader>
    );
  }
