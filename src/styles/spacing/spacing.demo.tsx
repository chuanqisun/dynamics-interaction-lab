...
export class SpacingDemo extends React.Component<any, any> {
  ...
  render() {
    ...
    const showLettersOverlappingError =
      (this.state.designerSpacing < 0 || this.state.designerSpacing === 0) && this.state.itemUpper.name !== "object" && this.state.itemLower.name !== "object";
      ...
      <StyledSection>
        <div className="steps">
          <div className="step">
            ...
            {typographyData.map((itemUpper) => (
              <label className="typography-option" key={itemUpper.name}>
                <input
                  type="radio"
                  name="itemUpper"
                  checked={this.state.itemUpper.name === itemUpper.name}
                  onChange={() => this.onSelectItemUpper(itemUpper)}
                />
                {itemUpper.name}
              </label>
            ))}
          ...
          <div className="step">
            ...
            {typographyData.map((itemLower) => (
              <label className="typography-option" key={itemLower.name}>
                <input
                  type="radio"
                  name="itemLower"
                  checked={this.state.itemLower.name === itemLower.name}
                  onChange={() => this.onSelectItemLower(itemLower)}
                />
                {itemLower.name}
              </label>
            ))}
          ...
          <div className="step">
            ...
            <div className="number-item number-item--wrap ">
              ...
              {spacingRamp.map((value) => (
                <React.Fragment key={value}>
                  <input
                    type="radio"
                    id={"ramp-item" + value}
                    name="ramp-item"
                    className="ramp-item__radio"
                    checked={this.state.designerSpacing === value}
                    onChange={() => this.onSelectRampItem(value)}
                  />
                  <label className="ramp-item__label" htmlFor={"ramp-item" + value}>
                    {value}
                  </label>
                </React.Fragment>
              ))}
            ...
            <label className="number-item" onMouseOver={this.onMouseOverDesignerSpacing} onMouseLeave={this.onMouseLeaveDesignerSpacing}>
              <span className="number-prompt">{`${this.state.itemUpper.name === "object" ? "Bounding box" : "Baseline"} to ${
                this.state.itemLower.name === "object" ? "bounding box" : "ascender"
              }`}</span>
              <input
                id="designer-spacing"
                ...
                type="number"
                step="4"
                onBlur={this.onBlurSpacing}
                onFocus={this.onFocusDesignerSpacing}
                value={this.state.designerSpacing}
                onChange={this.onChangeDesignerSpacing}
              />
              px
            </label>
            <label className="number-item" onMouseOver={this.onMouseOverDeveloperSpacing} onMouseLeave={this.onMouseLeaveDeveloperSpacing}>
              ...
              <input
                id="developer-spacing"
                className="number-input"
                type="number"
                step="4"
                onBlur={this.onBlurSpacing}
                onFocus={this.onFocusDeveloperSpacing}
                value={developerSpacing}
                onChange={this.onChangeDeveloperSpacing}
              />
              px
            </label>
            {showLettersOverlappingError ? (
              <span className="number-item">
                <span className="error-message">
                  <ErrorIcon className="error-icon" />
                  &nbsp;This spacing is impossible because the letters will overlap. You must to increase the spacing.
                </span>
              </span>
            ) : null}
            {showBoundingBoxOverlappingError ? (
              <span className="number-item">
                <span className="error-message">
                  <ErrorIcon className="error-icon" />
                  &nbsp;This spacing is impossible because the bounding boxes will overlap. You must to increase the spacing.
                </span>
              </span>
            ) : null}
            {showRampNonStandardWarning ? (
              <span className="number-item">
                <span className="error-message">
                  <WarningIcon className="error-icon" />
                  &nbsp;This spacing is not a standard ramp item. We recommend using the ramp.
                </span>
              </span>
            ) : null}
          </div>
        ...
        <div className="illustration">
          ...
          <StyledDiv {...this.state} className="sample-spacer">
            <div className="designer-spacer">
              <label
                htmlFor="designer-spacing"
                className="designer-hover-receiver"
                onMouseOver={this.onMouseOverDesignerSpacing}
                onMouseLeave={this.onMouseLeaveDesignerSpacing}
              ></label>
            </div>
            <div className="developer-spacer">
              <label
                htmlFor="developer-spacing"
                className="developer-hover-receiver"
                onMouseOver={this.onMouseOverDeveloperSpacing}
                onMouseLeave={this.onMouseLeaveDeveloperSpacing}
              ></label>
            </div>
          </StyledDiv>
...
export const DemoItemTypography = (props: TypographyItemProps) => (
  <div
...
export const DemoItemObject = (props: any) => (
  <div
    className={props.className}
    style={{
      height: "128px",
