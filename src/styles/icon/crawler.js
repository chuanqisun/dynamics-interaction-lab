...
for (let i = 0; i < iconsArray.length; i++) {
  iconsMap[iconsArray[i].querySelector('.icon-name').textContent] = iconsArray[i].querySelector('.icon-value').textContent;
}

var text = `export const FullMdl2 = {
${Object.keys(iconsMap).map(key => (`  "${key}": "\\u${iconsMap[key]}",`)).join('\n')}
};`;
